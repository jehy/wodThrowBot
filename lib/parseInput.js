...
function parseRequest(str) {
  ...
  const {
    special, damage, action, sum,
  } = getProps(options);
  const difficulty = sum ? 0 : (parseInt(arr[4], 10) || 6);
  ...
  const task = getTask({
    diceNumber, special, damage, action, base, sum, difficulty,
  });
  return {
    diceNumber, special, damage, action, base, sum, difficulty, task,
  };
}
